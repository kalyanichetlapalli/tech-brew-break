...
const Puzzles = () => {
  const puzzles = [
    ...
    },
    {
      ...
      description:
        "Fix the promise chain and make this async code work properly",
      difficulty: "Medium",
      category: "JavaScript",
      icon: Bug,
      color: "text-yellow-500",
    ...
    },
    {
      ...
      description:
        "Complete this recursive function to solve the tower of Hanoi",
      difficulty: "Hard",
      category: "Algorithms",
      icon: Brain,
      color: "text-red-500",
      ...
      <div className="container mx-auto px-4 py-12">
        ...
        <div className="text-center mb-12 animate-fade-in">
          ...
          <p className="text-xl text-muted-foreground max-w-2xl mx-auto">
            Sharpen your problem-solving skills with these interactive coding
            challenges
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {puzzles.map((puzzle, index) => {
            ...
            return (
              <Card
                key={puzzle.id}
                className="p-6 hover:shadow-card transition-all hover:scale-105 group border-2 hover:border-primary/50 animate-scale-in"
                style={{ animationDelay: `${index * 0.1}s` }}
              >
                <div className="flex items-start justify-between mb-4">
                  <div
                    className={`w-12 h-12 rounded-lg bg-primary/10 flex items-center justify-center group-hover:bg-brew-gradient transition-colors`}
                  >
                    <Icon
                      className={`w-6 h-6 ${puzzle.color} group-hover:text-white transition-colors`}
                    />
                  </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  >
                    <Link to={`/puzzle/${puzzle.id}`}>Solve →</Link>
                  </Button>
                </div>
              </Card>
            );
